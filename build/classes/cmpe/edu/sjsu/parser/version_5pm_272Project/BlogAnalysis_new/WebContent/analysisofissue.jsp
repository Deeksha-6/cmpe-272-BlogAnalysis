    ...
    <div class="container">
		...
		rs = st
				.executeQuery("SELECT ROUND(SUM(result.PositiveScore), 2) PositiveScore, ROUND(ABS(SUM(result.NegativeScore)), 2) NegativeScore FROM (" + 
								"SELECT SUM(c.Score) PositiveScore, 0 NegativeScore FROM blogcomments c WHERE c.topicid = \"" + idd[1] + "\" AND c.Score > 0  " + 
								"UNION ALL " + 
								"SELECT 0 PositiveScore, SUM(c.Score) NegativeScore FROM blogcomments c WHERE c.Score < 0 AND c.TopicId = \"" + idd[1] + "\"" +
							  ") result");
